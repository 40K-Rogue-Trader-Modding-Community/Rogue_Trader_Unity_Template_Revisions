...
namespace Kingmaker.Editor.UIElements.Custom.Properties
{
	...
	{
		public TraitsPartElement(SerializedProperty prop)
		{
			m_Property = prop;
		...
		private LocalizedString LocString => PropertyResolver.GetPropertyObject<LocalizedString>(m_Property);
		...
		private void FillStringTraits(VisualElement root)
		{
			...
			string[] stringTraits = LocString.GetStringTraits();
		...
		private void FillLocaleTraits(Locale locale)
		{
			...
			string[] localeTraits = LocString.GetLocaleTraits(locale);
		...
		private VisualElement GetButton(string[] data, bool isString, string trait)
		{
			...
			{
				isChecked = !isChecked;
				OnLocaleTraitClick(LocString, m_Property, btn, isString, trait); 
