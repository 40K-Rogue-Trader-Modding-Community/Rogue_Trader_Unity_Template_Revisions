...
namespace Kingmaker.Editor.Blueprints.Elements
{
    public class ReferenceField : BaseField<string>
    {
        ...
        public SerializedProperty CopyPasteProperty;
        ...
        public override void SetValueWithoutNotify(string newValue)
        {
            ...
            if (!flag)
                return;
            Blueprint = string.IsNullOrEmpty(newValue) ? 
                null : BlueprintsDatabase.LoadById<BlueprintScriptableObject>(value);
        ...
        internal void ShowObjectSelector()
        {
            ...
            BlueprintPicker.ShowAssetPicker(ObjectType, null, b => value = b?.AssetGuid ?? "", Blueprint);
        } 
        ...
        public void UpdateDisplay() => m_ReferenceFieldDisplay.Update();
        ...
        private class ReferenceFieldDisplay : VisualElement
        {
            ...
            public void Update()
            {
                m_ObjectIcon.image = m_ReferenceField.Blueprint ? OwlcatEditorStyles.Instance.BlueprintItemIcon: null;
                var n= m_ReferenceField.Blueprint == null
                    ? string.IsNullOrEmpty(m_ReferenceField.value)
                        ? "None"
                        : "Missing " + m_ReferenceField.value
                    :m_ReferenceField.Blueprint.name;
                m_ObjectLabel.text =
                    $"{n} [{(m_ReferenceField.Blueprint?.GetType() ?? m_ReferenceField?.ObjectType)?.Name}]";
            }
            ...
            protected override void ExecuteDefaultActionAtTarget(EventBase evt)
            {
                ...
                else if (evt.eventTypeId == EventBase<KeyDownEvent>.TypeId())
                {
                    ...
                    else if (keyDownEvent1 is {keyCode: KeyCode.Delete or KeyCode.Backspace})
                    {
                        OnKeyboardDelete();
                    }
                    else
                    {
                        OnKeyboardCopyPaste(evt as KeyDownEvent);
                    }
                }
            ...
            {
                var bp = m_ReferenceField.Blueprint;
            ...
            private void OnKeyboardCopyPaste(KeyDownEvent keyDownEvent)
            {
                if (keyDownEvent == null || m_ReferenceField.CopyPasteProperty == null)
                    return;
                
                if (keyDownEvent.ctrlKey && keyDownEvent.keyCode == KeyCode.C)
                {
                    CopyPasteController.CopyProperty(m_ReferenceField.CopyPasteProperty, null);
                    keyDownEvent.StopPropagation();
                }
                else if (keyDownEvent.ctrlKey && keyDownEvent.keyCode == KeyCode.V)
                {
                    var type = CopyPasteController.GetPasteableType(m_ReferenceField.CopyPasteProperty);
                    
                    if (CopyPasteController.IsSuitableForPaste(type) && 
                        CopyPasteController.PasteProperty(type, m_ReferenceField.CopyPasteProperty))
                    {
                        m_ReferenceField.CopyPasteProperty.serializedObject.ApplyModifiedProperties();
                        m_ReferenceField.CopyPasteProperty.serializedObject.Update();
                        keyDownEvent.StopPropagation();
                    }
                }
            }
            ...
            private UnityEngine.Object DNDValidateObject()
            {
                ...
                if(dragged is BlueprintEditorWrapper wrapper)
                {
                    if (m_ReferenceField.ObjectType.IsInstanceOfType(wrapper.Blueprint))
                        return dragged;
                }
                return null; 
            ...
            private void OnDragPerform(EventBase evt)
            {
                ...
                if (!(@object != (UnityEngine.Object)null))
                    return;
                DragAndDrop.visualMode = DragAndDropVisualMode.Generic;
                m_ReferenceField.value =BlueprintEditorWrapper.Unwrap<BlueprintScriptableObject>(@object)?.AssetGuid??"";
