...
namespace Kingmaker.Editor.UIElements.ValuePicker
{
	public class TypePicker : ValuePicker<Type>
	{
	    ...
	    public static void Button(
		    string buttonText,
		    Func<ValuesContainer<Type>> valuesContainerCollector,
		    Action<Type> callback,
		    bool showNow = false,
		    params GUILayoutOption[] options)
	    {
	    ...
	    public static VisualElement CreatePickerButton(string buttonText, Func<ValuesContainer<Type>> valuesContainerCollector, Action<Type> callback)
		    => CreateButton(GetWindow<TypePicker>, buttonText, valuesContainerCollector, callback);
		...
		{
			return ClassNames.GetClassName(value);
		}
	}
}